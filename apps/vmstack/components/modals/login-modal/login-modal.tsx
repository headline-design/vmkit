...
import { AVMWalletsScreen, ErrorScreen, EVMWalletsScreen, LoadingScreen, SubstrateWalletsScreen, SVMWalletsScreen } from "../connect/connect-screens";
import { ChainPolkadot, ChainSolana } from "@/icons/chains";
  ...
  return (
    <>
      ...
      <Button
        ...
        rounded
        full
        icon={<ChainSolana className="h-5 w-5" />}
        text="Solana"
        onClick={() => handleShowWallets("SVM-wallets")}
      />
      <Button
        variant="outline"
        rounded
        full
        icon={<IconEthereum className="h-5 w-5" />}
        text="Ethereum"
        onClick={() => handleShowWallets("EVM-wallets")}
      />
      <Button
        variant="outline"
        rounded
        full
        icon={<ChainPolkadot className="h-5 w-5" />}
        text="Substrate"
        onClick={() => handleShowWallets("Substrate-wallets")}
      />
    </>
  );
};
...
const SuccessScreen = () => (
  <div className="flex flex-col items-center justify-center p-4 gap-2">
    ...
    <p className="text-sm text-gray-500">You are now connected to VMkit</p>
...
export const LoginModalHelper = ({
  ...
  const renderScreen = () => {
    switch (state.currentScreen) {
      ...
      case "SVM-wallets":
        return <SVMWalletsScreen />;
      case "EVM-wallets":
        return <EVMWalletsScreen />;
      case "Substrate-wallets":
        return (
          <SubstrateWalletsScreen />
        );
      case "loading":
        return (
          <LoadingScreen
            ...
            {state.currentScreen === "default"
              ...
              : state.currentScreen === "socials"
                ...
                : state.currentScreen === "AVM-wallets"
                  ? "Connect AVM Wallet"
                  : state.currentScreen === "SVM-wallets"
                    ? "Connect SVM Wallet"
                    : state.currentScreen === "EVM-wallets"
                      ? "Connect EVM Wallet"
                      : state.currentScreen === "Substrate-wallets"
