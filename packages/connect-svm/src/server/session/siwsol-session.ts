...
export const APP_NAME = "Next Siwsol App";
export const COOKIE_NAME = "web3SVMSession";

// Check for required environment variables
if (typeof process !== "undefined" && process.env && typeof window === "undefined") {
  if (!process.env.SESSION_SECRET) {
    console.error("SESSION_SECRET cannot be empty for SIWSol session.");
  }
}
...
export interface ISiwsolSession extends IBaseSession {
  chainId?: number;
}

export class SiwsolSession extends BaseSession<ISiwsolSession> {
  chainId?: number;
  protected cookieName = COOKIE_NAME;
  protected sessionOptions = SESSION_OPTIONS;

  constructor(session?: ISiwsolSession) {
    super(session);
    this.chainId = session?.chainId;
  }

  static async fromCookies<S extends BaseSession>(
    this: { new (session?: ISiwsolSession): S }, // Remove `abstract`
    ...
    const sessionData = await unsealData<ISiwsolSession>(
      sessionCookie,
      SESSION_OPTIONS
    );
